...
export default function LocalProvidersTab() {
  ...
  // Persist default local base URLs for enabled local providers that do not yet have one.
  useEffect(() => {
    filteredProviders.forEach((provider) => {
      if (!provider.settings.enabled || provider.settings.baseUrl) {
        return;
      }
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (provider.settings.baseUrl) {
                            fetchOllamaModels(provider.settings.baseUrl);
                          }
                        }}
